...
{               
        ...
        $query1 = "UPDATE garant_externe SET nom=:nom, prenom=:prenom, id_card_number=:id_card_number, id_card_validity=:id_card_validity, tel=:tel, email=:email, proprietaire_local=:proprietaire_local, tel_proprietaire=:tel_proprietaire, date_naissance=:date_naissance, lieu_naissance=:lieu_naissance, profession=:profession, situation_matrimoniale=:situation_matrimoniale, nombre_enfants=:nombre_enfants, genre=:genre, ville=:ville, quartier=:quartier, pays=:pays where id_garant_ex = '$id' ";
        ...
        $sql1 = $db->prepare($query1);
            ...
            $sql1->execute();
                                    ...
                                    if($sql1)
                                    {
                                        ?>
                                        <script>
                                            alert('Garant a été bien modifié.');
                                                  window.location.href='<?=$garant['option2_link']?>';
                                        </script>
                                        <?php
                                    }
                                    ...
                                    {       
                                      ?>
                                        <script>
                                            alert('Garant n\'a pas été modifié.');
                                            window.location.href='modifier_garant_externe.php?id=<?=$id?>';
                                        </script>
                                        <?php
