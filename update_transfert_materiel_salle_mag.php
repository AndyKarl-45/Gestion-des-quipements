...
//                                          echo $nom_salle_dst.'</br>';
    ...
    $sql="SELECT * FROM demande_materiel where id_ask_mat='$id_ask_mat' ";
    $stmt = $db->prepare($sql);
    $stmt->execute();
...
//                                          echo $nom_salle_dst.'</br>';
    ...
    $query = "SELECT * from demande_materiel where id_materiel= '$id_materiel' and id_salles='$id_salles_src' and id_ask_mat='$id_ask_mat' ";
...
//           echo $prix_total_f;
                ...
                $query1 = "UPDATE demande_materiel SET quantite=:quantite where  id_materiel= '$id_materiel' and id_salles='$id_salles_src' and id_ask_mat='$id_ask_mat' ";
...
//             if($cnt==0){
                ...
                $date_valide='N/A';
                $query = " INSERT INTO demande_equipement_sal (id_salles_src,nom_salle_src,id_salles_dst,nom_salle_dst,date_debut,date_valide,etat,id_ask_eq) 
                     VALUES (:id_salles_src,:nom_salle_src,:id_salles_dst,:nom_salle_dst,:date_debut,:date_valide,:etat,:id_ask_eq)";
                ...
                $sql->bindParam(':id_ask_eq', $id_ask_eq);
                $sql->execute();


                $sql="SELECT max(id_ask_eq_sal) as total FROM demande_equipement_sal  ";
                $stmt = $db->prepare($sql);
                $stmt->execute();
                ...
                $sql = "INSERT INTO demande_materiel_sal (id_salles_src,id_materiel,nom_salle_src,id_salles_dst,nom_salle_dst,date_debut_mat,id_ask_eq_sal,quantite,etat,id_ask_mat )
        VALUES(:id_salles_src,:id_materiel,:nom_salle_src,:id_salles_dst,:nom_salle_dst,:date_debut,:id_ask_eq_sal,:quantite,:etat,:id_ask_mat )";
                $req = $db->prepare($sql);
                ...
                $req->bindParam(':id_ask_mat', $id_ask_mat);
                $req->execute();
...
//             }
                if($quantite_total==0){
                    $query = "DELETE FROM demande_materiel where  id_materiel= '$id_materiel' and id_salles='$id_salles_src' and id_ask_mat='$id_ask_mat'";
...
//                $req->execute();
                ?>
                <script>
                    alert('Equipement  pas assez suffisant !!!.');
            ...
            ?>
            <script>
                alert('Impossible: Equipement  est à 0 unité !!!.');
                window.location.href='details_salle.php?id=<?php echo $ids_s?>';
            </script>
            <?php
