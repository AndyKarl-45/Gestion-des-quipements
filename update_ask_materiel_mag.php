...
$id = count($id_materiel);
 ...
 if($id_materiel[$j]!=""){
  ...
  //                                   $req->execute();
         $sql3 = "SELECT DISTINCT count(id_materiel) as total from demande_materiel where id_materiel = '$id_materiel[$j]' and id_ask_eq='$id_ask_eq' and receveur = '$receveur' ";
    ...
    $date_now=date('y-m-d');
       ...
       $query1 = "UPDATE demande_materiel SET quantite=:quantite where id_materiel = '$id_materiel[$j]' and id_ask_eq='$id_ask_eq' and receveur = '$receveur' ";
        ...
        { 
           switch ($emetteur){
                        case 'C';
                           ?>
                            <script>
                                alert('Demande a  été modifié.');
                                           window.location.href='modifier_transfert_mag.php?id=<?=$id_ask_eq?>';
                            </script>
                            <?php
                            break;
                        case 'E';
                           ?>
                            <script>
                               alert('Demande a  été modifié.');
                                           window.location.href='modifier_transfert_mag_save.php?id=<?=$id_ask_eq?>';
                            </script>
                            <?php
                            break;
                        case 'D';
        ...
        ?>
                            <script>
                                alert('Demande n\'a pas été modifié.');
                                 window.location.href='ajouter_ask_mag.php?idi=<?=$id_ask_eq?>&ids=<?=$receveur?>';
                            </script>
                            <?php
