            ...
            <!--                Main Body-->
                                            ...
                                            <div class="table-responsive">
                                                <form method="post" action="" >
                                                    ...
                                                    <?php
                                                        ...
                                                        $query = "SELECT * from garant_externe where open_close!='1' ";
                                                        $q = $db->query($query);
                                                        while($row = $q->fetch())
                                                        {
                                                          $id_garant_ex = $row['id_garant_ex']; 
                                                            $nom = $row['nom'];
                                                            $prenom = $row['prenom'];
                ...
                <td align="center">
                    ...
                    <div class="btn-group mr-2" role="group" aria-label="First group">
                         <a class="btn btn-primary"  href="details_garant_externe.php?id=<?= $id_garant_ex; ?>" title="view"
                            style="background-color: transparent">
                                <i  style="color: green" class="fas fa-eye"></i> 
                            </a>
                        </div>
                    <div class="btn-group mr-2" role="group" aria-label="First group">
                         <a class="btn btn-warning" href="modifier_garant_externe.php?id=<?=$id_garant_ex;?>" title="Modifier"
                            style="background-color: transparent">
                                <i style="color: orange" class="fas fa-pen"></i> 
                            </a>
                       </div>
                     <div class="btn-group mr-2" role="group" aria-label="First group">
                             <a class="btn btn-danger" type="button" data-toggle="modal" data-target="#verifier_delete_garant_externe<?= $id_garant_ex; ?>" title="supprimer"  style="background-color: transparent">
