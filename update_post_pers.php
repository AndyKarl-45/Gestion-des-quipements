...
{               
        ...
        $query1 = "UPDATE personnel SET nom=:nom, prenom=:prenom, id_card_number=:id_card_number, id_card_validity=:id_card_validity, tel=:tel, email=:email, date_naissance=:date_naissance, lieu_naissance=:lieu_naissance, profession=:profession, situation_matrimoniale=:situation_matrimoniale, nombre_enfants=:nombre_enfants, genre=:genre, id_quartier=:id_quartier, id_ville=:id_ville, id_pays=:id_pays where id_personnel = '$id' ";
        ...
        $sql1 = $db->prepare($query1);
            ...
            $sql1->execute();
                                    ...
                                    if($sql1)
                                    {
                                        ?>
                                        <script>
                                            alert('Postulant a été bien modifié.');
                                    ...
                                    {       
                                      ?>
                                        <script>
                                            alert('Postulant n\'a pas été modifié.');
                                            window.location.href='modifier_postulant.php?id=<?=$id?>';
                                        </script>
                                        <?php
